...
Module DefTupleOf.
  ...
  (* Uses results on seq (without size constraints), therefore redoing all proofs *)
  by rewrite size_rev size_map size_rev //=; case : t=> tval /eqP -> . 
  Qed. 


  (* Text: Let us examine what happens if we try to unify the left hand 
           side of the size_tuple equation with the redex (size (rev t)),
          using the following toolkit:
  *)
  Notation "X (*...*)" := 
  ...
  Fail Check forall T n (t : n.-tuple T),
      ...
      [unify LHS with RDX].
      (* Extraction of LHS ... not convincing (?)*)  
